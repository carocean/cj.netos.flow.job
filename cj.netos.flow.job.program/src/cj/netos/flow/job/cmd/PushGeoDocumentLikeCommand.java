...
@CjService(name = "/geosphere/document/like.mq#pushGeoDocumentLike")
public class PushGeoDocumentLikeCommand extends PushGeoFlowJobBase {
    @Override
    public void command(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws RabbitMQException, RetryCommandException, IOException {
        ...
        JPushFrame frame = new JPushFrame("likeDocument /geosphere/receptor gbera/1.0", bb);
        String creator=doc.getCreator();
        frame.parameter("docid", docid);
        frame.parameter("creator",creator);
        frame.head("sender-person", liker);

        Map<String, List<String>> destinations = getDestinations(doc.getReceptor(), GeoPushType.pushLike, creator);
